...
jQuery.Class("Vtiger_Helper_Js",{
	...
	showDesktopNotify : function(customParms) {
	
			 PNotify.desktop.permission();
			(new PNotify({
				title: customParms.title,
				text: customParms.text,
				type: 'info',
				hide: false,
				desktop: {
					desktop: true
    ...
    */ 
	...
	showHorizontalTopScrollBar : function() {
		...
		jQuery('.bottomscroll-div', container).css('width', jQuery('.topscroll-div', container).outerWidth());
                                    var TableHeight = window.outerHeight - 280;
		 if (jQuery('.listViewEntriesDiv').height() > TableHeight) {
                app.showScrollBar( jQuery('.listViewEntriesDiv'), {'height':(TableHeight - 10),'railVisible':'true','size':'10px'});
            }
