...
class Vtiger_Record_Model extends Vtiger_Base_Model {
	...
	public static function widesearch($searchKey, $searchmodule, $db=false) {
                            
		if($searchmodule==='HelpDesk') {

			$query = "SELECT label, description, crmid, setype, createdtime FROM vtiger_crmentity WHERE (label LIKE ? OR description LIKE ? OR crmid LIKE ?) AND vtiger_crmentity.deleted = 0 AND setype = 'HelpDesk' ";
			$params = array("%$searchKey%","%$searchKey%","%$searchKey%");
		}
			   
		if($searchmodule ==='Contacts') {

			$query = "SELECT contact.email, contact.phone, contact.mobile, crmen.crmid, crmen.label, crmen.setype, crmen.createdtime FROM vtiger_contactdetails AS contact INNER JOIN vtiger_crmentity AS crmen ON contact.contactid = crmen.crmid WHERE (crmen.label LIKE ? OR contact.email LIKE ? OR contact.phone LIKE ? OR contact.mobile LIKE ?) AND crmen.setype = 'Contacts' AND crmen.deleted = 0";
			$params = array("%$searchKey%","%$searchKey%","%$searchKey%", "%$searchKey%");
		}
		if($searchmodule ==='Accounts') {

			$query = "SELECT account.accountname, account.phone, account.fax, crmen.crmid, crmen.label, crmen.setype, crmen.createdtime FROM vtiger_account AS account INNER JOIN vtiger_crmentity AS crmen ON account.accountid = crmen.crmid WHERE (crmen.label LIKE ? OR account.accountname LIKE ? OR account.phone LIKE ? OR account.fax LIKE ?) AND crmen.setype = 'Accounts' AND crmen.deleted = 0";
			$params = array("%$searchKey%","%$searchKey%","%$searchKey%", "%$searchKey%");
		}
		if($searchmodule ==='Products') {
			$query = 'SELECT product.productname, product.productcode, crmen.crmid, crmen.description, crmen.setype, crmen.createdtime FROM vtiger_products AS product INNER JOIN vtiger_crmentity AS crmen ON product.productid= crmen.crmid WHERE (product.productname LIKE ? OR product.productcode LIKE ? OR crmen.description LIKE ?) AND crmen.setype = "Products" AND crmen.deleted = 0';
			$params = array("%$searchKey%","%$searchKey%","%$searchKey%");
		}
		if($searchmodule ==='Quotes' || $searchmodule ==='SalesOrder'  || $searchmodule === 'Project')
		{
			$tablename = Array(
				"Quotes" => "vtiger_quotescf",
				"SalesOrder" => "vtiger_salesordercf",
				"Project" => "vtiger_projectcf"
			);
			$moduleid = Array(
				"Quotes" => "quoteid",
			...
			$modulefields_res = $db->pquery('SELECT columnname FROM vtiger_field WHERE tablename = ?', Array($tablename[$searchmodule]));
			if($db->num_rows($modulefields_res)) {

				$modulefields  = Array();
				for ($i=0; $i<=$db->num_rows($modulefields_res) - 1 ; $i++)
				{
					$modulefields[$i]= $db->query_result($modulefields_res, $i);
				} 

				$query = "SELECT crmid, setype, createdtime FROM vtiger_crmentity INNER JOIN  $tablename[$searchmodule] ON $tablename[$searchmodule].$moduleid[$searchmodule]= vtiger_crmentity.crmid WHERE ($tablename[$searchmodule].$modulefields[0] LIKE ? ";
				$params = array("%$searchKey%");
		
				foreach (array_slice($modulefields, 1) as $col)
				{
					$query .= "  OR $tablename[$searchmodule].$col LIKE ?";   
					$params[] = "%$searchKey%";
				}
				$query .=") AND vtiger_crmentity.setype ='$searchmodule' AND vtiger_crmentity.deleted = 0";
				//vtiger_quotescf.$SiteLocationfield[$i] LIKE ?
				//AND crmen.setype = 'Products' AND crmen.deleted = 0
			}
		}
		// else
		// {
		...
		// }
		$searchparams =array("query"=>$query, "params"=>$params);         
		return $searchparams;		   
	}
	...
	public static function getSearchResult($searchKey, $module=false) {
		...
		$searchparms = self::widesearch($searchKey, $module, $db);
		$result = $db->pquery($searchparms["query"], $searchparms["params"]);  
