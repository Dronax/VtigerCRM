		...
		<td>
			...
			{if $ITEM_DISCOUNT_AMOUNT_EDITABLE || $ITEM_DISCOUNT_PERCENT_EDITABLE}
				<div>
					<span>(-)&nbsp;
						<strong><a href="javascript:void(0)" class="individualDiscount">{vtranslate('LBL_DISCOUNT',$MODULE)}
							...
							<span id="discountTotal{$row_no}" class="discountTotal">{if $data.$discountTotal}{$data.$discountTotal}{else}0{/if}</span>
					</span>
				</div>
				...
				<div style="width:200px;">
					<strong>{vtranslate('LBL_UNIT_AFTER_DISCOUNT',$MODULE)} :  </strong>
					<span id="totalAfterDiscount{$row_no}" class="totalAfterDiscount">{if $data.$totalAfterDiscount}{$data.$totalAfterDiscount}{else}0{/if}</span>
				</div>
