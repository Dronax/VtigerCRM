        ...
        <table class="table table-bordered lineItemsTable" style = "margin-top:15px">
            ...
            <tbody>
                <tr>
                    ...
                    {/if}
                    <td class="lineItemFieldName">
                        <strong class="pull-right">{vtranslate('LBL_TOTAL_BEFORE_DISCOUNT',$MODULE_NAME)}</strong>
                    </td>
                    {if $MARGIN_VIEWABLE}
                        <td class="lineItemFieldName">
                        ...
                        {if $LIST_PRICE_VIEWABLE}
                            <td style="white-space: nowrap;">
                                ...
                                {if $ITEM_DISCOUNT_AMOUNT_VIEWABLE || $ITEM_DISCOUNT_PERCENT_VIEWABLE}
									...
									{elseif $LINE_ITEM_DETAIL["discount_type$INDEX"] == 'percentage'} {$LINE_ITEM_DETAIL["discount_percent$INDEX"]} % {vtranslate('LBL_OF',$MODULE_NAME)} {$LINE_ITEM_DETAIL["listPrice$INDEX"]} = {$LINE_ITEM_DETAIL["discountTotal$INDEX"]}
									{/if}"}
                                        (-)&nbsp; <strong><a href="javascript:void(0)" class="individualDiscount inventoryLineItemDetails" tabindex="0" role="tooltip" id ="example" data-toggle="popover" data-trigger="focus" title="{vtranslate('LBL_DISCOUNT',$MODULE_NAME)}" data-content="{$DISCOUNT_INFO}">{vtranslate('LBL_DISCOUNT',$MODULE_NAME)}</a> :  {$LINE_ITEM_DETAIL["discountTotal$INDEX"]}</strong>
                                    </div>
                                {/if}
                                <div>
                                    <strong>{vtranslate('LBL_UNIT_AFTER_DISCOUNT',$MODULE_NAME)} : {$LINE_ITEM_DETAIL["totalAfterDiscount$INDEX"]}</strong>
                                </div>
